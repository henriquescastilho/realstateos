...
- Wave 13 tasks 110-113: ADK eval framework (50 golden scenarios, 5 agents, EvalSuite+EvalReport+CI regression guard), Python unit tests (104 tests — billing/dates/webhook/charge services + agent_tasks/audit_log/contracts repos), Node.js unit tests (327 tests passing — calculator, reconciliation, cpf, classifier, statement pure functions + all Zod validator schemas), Python integration tests (42 tests — real PostgreSQL+Redis via testcontainers, skip when Docker unavailable, covers full billing cycle/payment reconciliation/agent task lifecycle)
...
- Wave 12 COMPLETE (101-110): ContractQAAgent (`app.agents.contract_qa_agent`) — LlmAgent RAG over pgvector `contract_chunks`. Tools: `search_contract_chunks(db, contract_id, query, limit)` (pgvector cosine + full-text fallback), `get_contract_metadata(db, contract_id)`. Non-ADK fallback: `answer_question_fallback()` returns raw chunks + metadata dict.
- Wave 13 task 111: Python unit test suite — 8 test files created targeting ~80% coverage on `apps/api/app/services/` and `apps/api/app/repositories/`. Tests: `tests/services/test_billing_service.py` (11 tests: generate_monthly_rent_charge), `tests/services/test_dates_utils.py` (9 tests: resolve_due_date + move_to_next_business_day), `tests/services/test_webhook_service.py` (15 tests: _compute_signature + verify_webhook_signature + dispatch_webhook_event), `tests/services/test_charge_service.py` (12 tests: create_monthly_charges + get/update/generate_boleto/consolidate), `tests/repositories/test_agent_tasks_repo.py` (34 tests: full CRUD + bulk ops), `tests/repositories/test_audit_log_repo.py` (16 tests: append/get/list/CSV/JSON export), `tests/repositories/test_contracts_repo.py` (12 tests: list/get/expiring with tenant isolation + soft delete). All use `db_session` fixture (SQLite in-memory). External deliveries patched via `unittest.mock.patch`.

- Anomaly detector: `app.ml.anomaly_detector` — `PaymentAnomalyDetector`. Features: amount, amount_ratio vs median, days_since_last, hour_utc, day_of_week, is_new_payer, is_round_amount. IsolationForest (sklearn, contamination=0.05) with rule-based fallback. `detect_and_store(db, payment_id)` → upserts `payment_anomalies` table. `get_detector(org_id)` cached singleton.
- Default predictor: `app.ml.default_predictor` — `DefaultPredictor` class. Features: avg/max days late, partial_payment_rate, late_payment_rate, payment_count, maintenance_ticket_count, months_as_renter. sklearn LogisticRegression when available (≥20 samples), heuristic weighted-average fallback. `score_all_and_store(db, org_id)` → upserts `renter_default_scores` table. `get_predictor(org_id)` cached singleton.

- Charge composer: `app.agents.billing_agent.charge_composer` — `ChargeCompositionAgent` LlmAgent + `compose_charge_rule_based()` fallback. Tools: `get_contract_terms()`, `get_applicable_index()`, `get_late_charges()`. Output: `ChargeComposition` with `line_items` (base_rent/index_adjustment/late_fee/discount/condominium) + `explanation_pt` (plain Portuguese). Anniversary-month detection for annual index adjustment.

- Maintenance cost estimator: `app.agents.maintenance_agent.cost_estimator` — `MaintenanceCostEstimatorAgent` LlmAgent + `estimate_cost_rule_based()` fallback. Tools: `find_similar_tickets(db, description, limit)` (pgvector + full-text fallback), `get_ticket_context(db, ticket_id)`. Output: `CostEstimate` (min/expected/max, confidence low/medium/high, Portuguese explanation). Category-based range fallback for 10 categories.

- Renewal recommender: `app.agents.portfolio_agent.renewal_recommender` — `ContractRenewalRecommenderAgent` LlmAgent + `process_expiring_contracts()` rule-based batch. `get_expiring_contracts(db, days_ahead)` with payment aggregations. `recommend_rule_based()`: renew (<20% late)/renegotiate (≥20% or unpaid)/terminate (≥3 late AND >30d delay). `compose_renewal_email()` drafts Portuguese owner email. `RenewalRecommendation` dataclass.

- NL query API: `app.routes.nl_query` — `POST /query`. LlmAgent text-to-SQL (ADK) with template-based fallback (5 patterns: expiring/overdue/revenue/maintenance/properties). SQL validation: SELECT-only, no write keywords, no system tables. Tenant isolation enforced in generated SQL. Returns `NLQueryResponse`: question, sql, columns, rows, explanation_pt, model_used. Registered in router.py.

- Document agent: `app.agents.document_agent` — `DocumentComparisonAgent` LlmAgent. Tools: `extract_contract_clauses(pdf_path)` (pdfplumber→PyPDF2→text fallback, regex clause parsing), `diff_clauses(v1, v2)` (title-matched unified diff), `summarize_changes(diffs)` (PT summary). `compare_contracts_fallback()` non-ADK mode. severity classification (low/medium/high) for changes.

- Photo analyzer: `app.agents.maintenance_agent.photo_analyzer` — `analyze_photo_with_gemini(source, api_key)` using `gemini-1.5-flash` multimodal. Returns `PhotoAnalysisResult`: damage_category (11 categories), severity (low/medium/high/critical), urgency (PT label), description_pt, suggested_title, auto_fields for ticket. `analyze_ticket_photos()` batch (worst-case severity). Graceful fallback when genai unavailable or no API key.

- ADK eval framework: `apps/api/tests/agents/eval/` — `framework.py` (AgentEvaluator, EvalSuite, EvalReport; PASS_THRESHOLD=0.80, SUITE_PASS_THRESHOLD=0.95, CI_ALERT_DROP_PCT=0.05), `golden_dataset.py` (50 scenarios across 5 agent types: onboarding×10, payments×10, comms×6, maintenance×7, portfolio×5 + 12 extra), `test_eval_suite.py` (pytest CI runner: overall≥95%, per-agent≥70%, tool_selection≥80%, output_quality≥80%, escalation_precision≥75%, baseline regression guard). Run: `pytest apps/api/tests/agents/eval/test_eval_suite.py -v`. Save baseline: add `--save-baseline` flag.

## Last Updated
Loop: 90 | Timestamp: 2026-03-14
